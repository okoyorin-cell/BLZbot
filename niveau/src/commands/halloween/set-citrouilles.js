...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('set-citrouilles')
        .setDescription('[Admin] Définir le nombre de citrouilles d\'un membre.')
        ...
        .addIntegerOption(option =>
            option.setName('montant')
                .setDescription('Le nouveau nombre de citrouilles.')
                .setRequired(true)
                .setMinValue(0)),
    ...
    async execute(interaction) {
        ...
        db.prepare('UPDATE event_users SET citrouilles = ? WHERE user_id = ?').run(amount, targetUser.id);

        await interaction.reply({
            content: `Le nombre de citrouilles de **${targetUser.username}** a été défini sur **${amount.toLocaleString('fr-FR')}**. `,
            flags: 64
        });
    },
};